...
static
CLUST get_fat (
	FATFS *fs,	/* 1:IO error, Else:Cluster status */
...
static
DWORD clust2sect (
	FATFS *fs,				/* !=0: Sector number, 0: Failed - invalid cluster# */
...
)
{
	...
	dj->sect = clst ? clust2sect(fs, clst) : fs->dirbase;	/* Current sector */
		...
		if ((fs->fptr % 512) == 0) {				/* On the sector boundary? */
			...
			if (!cs) {								/* On the cluster boundary? */
				clst = (fs->fptr == 0) ?			/* On the top of the file? */
					fs->org_clust : get_fat(fs, fs->curr_clust);
